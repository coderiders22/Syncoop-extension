        ...
        if (typeof obj[key] === 'object' && obj[key] !== null) {
            ...
            if (key == "$ref") {
                if (obj[key].startsWith("https://")) {
                    let response = syncRestRequest("/packages.middleware.pub.client.http.requestAPI.main", "POST", JSON.stringify({
                        url: obj[key],
                        method: "GET"
                    }), "application/json");
                    if (null != parentObj) {
    ...
    var ref = $(id)
        .jstree({
            ...
            "types": {
                ...
                "stringList": {
                    "icon": "../../../icons/textArr.svg",
                    "valid_children": []
                },
                "javaObject": {
                    "icon": "../../../icons/javaObject.svg",
                    "valid_children": []
                },
                "javaObjectList": {
                    "icon": "../../../icons/javaObjectArr.svg",
                    "valid_children": []
                },
                "integer": {
...
const FLOW_JS_TREE_CONFIG = {
    ...
    "types": {
        ...
        "ifelse": {
            "icon": "../../../icons/filesystem/ifelse.svg",
    ...
    if (!(node.type == "root" || node.type == "package" || node.type == "folder")) {
        ...
        items.newtab = {
            ...
            action: function (n) {
                ...
                if (node.type == "api") {
                    window.open("/files/gui/middleware/pub/server/ui/workspace/web/apiMaker/apiEditor.html?loadFile=files/" + dest + "." + node.type, "_blank");
                } else if (node.type == "flow") {
                    window.open("/files/gui/middleware/pub/server/ui/workspace/web/flowMaker/flowEditor.html?loadFile=files/" + dest + "." + node.type, "_blank");
                } else if (node.type == "service") {
                    window.open("/files/gui/middleware/pub/server/ui/workspace/web/CodeMirror-master/mode/clike/serviceEditor.html?loadFile=files/" + dest + "." + node.type, "_blank");
                } else if (node.type == "sql") {
                    window.open("/files/gui/middleware/pub/server/ui/workspace/web/CodeMirror-master/mode/sql/sqlEditor.html?loadFile=files/" + dest + "." + node.type, "_blank");
                } else if (node.type == "jdbc") {
                    window.open("/files/gui/middleware/pub/server/ui/workspace/web/CodeMirror-master/mode/properties/jdbcEditor.html?loadFile=files/" + dest + "." + node.type, "_blank");
                } else if (node.type == "jdbc" || node.type == "properties" || node.type == "graphql") {
                    window.open("/files/gui/middleware/pub/server/ui/workspace/web/CodeMirror-master/mode/properties/propertiesEditor.html?loadFile=files/" + dest + "." + node.type, "_blank");
                }
        ...
        if(node.type=="api" || node.type=="flow"){
            items.test = {
                ...
                submenu: {},
                action: function (n) {
                    window.open("/files/gui/middleware/pub/server/ui/oas/client.html?fqn=" + FQN);
                }
            };
            items.graphQL = {
                label: "GraphiQL",
                submenu: {},
                action: function (n) {
                    var aliasResp=syncRestRequest("/alias?fqn="+FQN+".main", "GET");
                    if(aliasResp.status="200"){
                        ...
                        if(null != payload.alias && payload.alias.startsWith("POST")){
                            ...
                            window.open("/files/gui/middleware/pub/server/ui/graphQL/client.html?endpoint="+endpoint);
                        }else{
...
function createPackageJstree(id) {
    ...
    var ref = $(id)
        .jstree({
            ...
            "types": {
                "#": {
                    "icon": "../../../icons/filesystem/unknown.png",
                    "valid_children": [
                        "root", "ui-root"
                    ]
                },
                ...
                "package": {
                    "icon": "../../../icons/myPackages.svg",
                    "valid_children": [
                        "folder"
                    ]
                },
                "gui-app": {
                    "icon": "../../../icons/gui-app.png",
                    "valid_children": [
                        "folder"
                    ]
                },
                "folder": {
                    "icon": "../../../icons/filesystem/folder.svg",
                    "valid_children": [
                        "folder", "service", "package", "api", "flow", "map", "transformer", "html", "js", "css", "jar", "jdbc", "sql", "properties", "csv", "txt"
                    ]
                },
                "service": {
                    "icon": "../../../icons/filesystem/cog.svg",
                    "valid_children": []
                },
                "map": {
                    "icon": "../../../icons/filesystem/arrow_switch.png",
                    "valid_children": []
                },
                "transformer": {
                    "icon": "../../../icons/filesystem/arrow_switch.png",
...
function pasteNode(source){
    if (source){
        var treeId = '#'+source;
        var output_tree = $(treeId).jstree(true);
        var length = output_tree.get_json('#').length;
        var textStr  = output_tree.get_buffer().node[0].text;
        var repeatFlag = false;
        var node;
        var existingValues;
        if(source == "output_schema_editor_jsTree"){
            existingValues = JSON.parse(localStorage.getItem(loadFile + "_outputJsTree"));
        }
        else{
            existingValues = JSON.parse(localStorage.getItem(loadFile + "_inputJsTree"));
        }

        $(existingValues).each(function(e, data){
            if (data.text == textStr){
                var nodeId = data.id;
                node = output_tree.get_node(nodeId);
                repeatFlag = true;
            }
        });
        ...
        if (repeatFlag){
            swal({
                    ...
                    text: "This element already exist. Do you want to overwrite?",
                    type: "warning",
                    showCancelButton: true,
                    confirmButtonColor: "#DD6B55",
                ...
                function(isConfirm){
                    if (isConfirm) {
                        output_tree.delete_node(node);
                        output_tree.paste('#', length);
                    }
                });
        }
        else{
...
function createItemInSchema() {
    ...
    if (NEW_ITEM_SEL) {
        ...
        if (NEW_ITEM_TYPE == "folder") {
            syncRestRequest("/packages.middleware.pub.server.browse.saveEmptyFolder.main?packageName=" +
        ...
        if (NEW_ITEM_TYPE == "flow" || NEW_ITEM_TYPE == "api" || NEW_ITEM_TYPE == "service" || NEW_ITEM_TYPE == "sql" || NEW_ITEM_TYPE == "jdbc") {
            ...
            if (thisNode.type == "api") {
                ...
                $("#middlewareCodeEditor").attr('src', "/files/gui/middleware/pub/server/ui/workspace/web/apiMaker/apiEditor.html?loadFile=" + path);
                // window.open("/files/gui/middleware/pub/server/ui/workspace/web/workspace.html?r=" + path, "_blank");
            } else if (thisNode.type == "service") {
                $("#middlewareCodeEditor").attr('src', "/files/gui/middleware/pub/server/ui/workspace/web/CodeMirror-master/mode/clike/serviceEditor.html?loadFile=" + path);
                ...
                $("#middlewareCodeEditor").attr('src', "/files/gui/middleware/pub/server/ui/workspace/web/CodeMirror-master/mode/sql/sqlEditor.html?loadFile=" + path);
            } else if (thisNode.type == "jdbc") {
                //let data = "";
                //asyncRestRequest("/files" + Servicepath + "/" + thisNode.text + "." + thisNode.type, data, "POST", function (result) {});
                $("#middlewareCodeEditor").attr('src', "/files/gui/middleware/pub/server/ui/workspace/web/CodeMirror-master/mode/properties/jdbcEditor.html?loadFile=" + path);
            }
...

            {
                ...
                if (elemId == null)
                    ...
                    if (buildName != null && buildName.trim().length > 0) {
                        ...
                        var response = null;
                        if ($("#import_environment").is(":checked")) {
                            response = syncRestRequest("/packages.middleware.pub.server.build.api.promoteBuild.main?" + qp + "&environment_id=" + $("#environment_id").val(), "POST", content, "application/json", "application/json");
                        } else {
                            response = syncRestRequest("/build?" + qp, "POST", content, "application/json", "application/json");
                        }
                        if (response.status == 200) {
                            var jObj = JSON.parse(response.payload);
...
function cloneProperties(newFlowName) {
    ...
    if (properties != null && properties.trim().length > 0) {
        ...
        syncRestRequest(propURLPath, "POST", properties, "application/text", "application/text");
    }
}
    ...
    asyncRestRequest("/packages.middleware.pub.environments.api.getEnvironments.main?start=0&length=100", null, "GET", function (response) {
        ...
        for (var i = 0; i < response.environments.length; i++) {
            ...
            if (null != response.environments[i].reason && response.environments[i].reason.trim() != "") {
                reason = '<div class="alert_mes"> <img src="/files/gui/middleware/pub/server/ui/assets/img/promotion_alert.svg">' + response.environments[i].reason + '</div>'
            }
...
function loadGroupsDP() {
    var response = syncRestRequest("/packages.middleware.pub.security.flow.getGroups.main", "GET", "");
...
function downloadContentOffline(text, fileName, mime) {
...
function getFileLocation(r) {
    let fileLocation = "";
    if (r.indexOf(".api") > 0) {
        fileLocation = "/files/gui/middleware/pub/server/ui/workspace/web/apiMaker/apiEditor.html?loadFile=" + r;

    } else if (r.indexOf(".flow") > 0) {
        fileLocation = "/files/gui/middleware/pub/server/ui/workspace/web/flowMaker/flowEditor.html?loadFile=" + r;
    } else if (r.indexOf(".service") > 0) {
        fileLocation = "/files/gui/middleware/pub/server/ui/workspace/web/CodeMirror-master/mode/clike/serviceEditor.html?loadFile=" + r;
    } else if (r.indexOf(".sql") > 0) {
        fileLocation = "/files/gui/middleware/pub/server/ui/workspace/web/CodeMirror-master/mode/sql/sqlEditor.html?loadFile=" + r;
    } else if (r.indexOf(".jdbc") > 0 || r.indexOf(".graphql") > 0) {
        fileLocation = "/files/gui/middleware/pub/server/ui/workspace/web/CodeMirror-master/mode/properties/jdbcEditor.html?loadFile=" + r;
    } else if (r.indexOf(".properties") > 0) {
        fileLocation = "/files/gui/middleware/pub/server/ui/workspace/web/CodeMirror-master/mode/properties/propertiesEditor.html?loadFile=" + r;
    } else if (r.indexOf(".map") > 0) {
        fileLocation = "/files/gui/middleware/pub/server/ui/workspace/web/transformer.html?loadFile=" + r;
    }
    return fileLocation;
}
...
function jsonToTable(json, itemKey, hideInternal, foundLoop, displayIndex, renderingPanel) {
    ...
    if (isInIframe()) {
        ...
        displayJSON(parent.document.getElementById(renderingPanel), finalJson, hideInternal, null);
    } else {
...
function serviceInputToSchema() {
    ...
    asyncRestRequest("/packages.middleware.pub.util.SyncloopIOtoSchema.main", JSON.stringify({
        ...
        $("#test_popup_id").attr("src", "/files/gui/middleware/pub/server/ui/workspace/web/api-service-ui-client.html");
