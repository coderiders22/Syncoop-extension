...
function getSystemResourcePath() {
    if (SDK_EMBEDDED) {
        ...
        return "/files/gui/middleware/pub/server/ui";
    }
}
...
function uploadFile(url, key, commaSepFileExts) {
    ...
    $("#middlewareFile").change(function () {
        ...
        let importedServiceResponse = null;

        function progressChecker(bar, percentage){
            const interval = setInterval(function () {
                ...
                if (progressStatus === "Success") {
                    ...
                    var list = importedServiceResponse.list;
                    openServicePopupForImport({
                        children: list,
                        type: "root",
                        text: "packages"
                    });
        ...
        asyncRestRequestV2(url, fd, "POST", function (response) {
            progressStatus = "Success";
            importedServiceResponse = response;
        }, function (error) {
        ...
        progressChecker(bar, percentage);
...
function openServicePopupForImport(value) {
    ...
    $("#service_tree_v").html("");
    var inputVal = addIconPath(value);

    var element = document.createElement("div");
    var deleteReferences = createReferenceJsTree(element, inputVal);
...
function createNewTenant(ref) {
    ...
    for (var i = 0; i < keywords.length; i++) {
        if (keywords[i] == text) {
            ...
            swal("Validation", capitalize(message), "error");
            return;
        }
    }
   ...
   // item_filed.classList.remove("packages_item_name_eb");
    ...
    if (null == localStorage.getItem("licenseCredits")) {
        location.href = "/packages.middleware.pub.tenant.createNew.main?tenantName=" + document.getElementById('tenantName').value + "&redirect=/files/gui/middleware/pub/server/ui/workspace/web/workspace.html";
    } else {
        let licenseCredits = localStorage.getItem("licenseCredits");
        localStorage.removeItem("licenseCredits");
        location.href = "/packages.middleware.pub.tenant.createNew.main?licenseCredits=" + licenseCredits + "&tenantName=" + document.getElementById('tenantName').value + "&redirect=/files/gui/middleware/pub/server/ui/workspace/web/workspace.html";
    }
...
function addService() {
    ...
    if (null != selectedNodeRef && (ref.get_node(selectedNodeRef).type == "function"
        ...
        swal("Invalid Change", "Select a service type step.", "error");
        return ;
    }
...
function openSmartAipopup() {
        asyncRestRequest(`/packages.middleware.pub.syncloopGPT.checkIfTokenExists.main`, null, 'POST', function (data) {
            if (data.checkToken) {
                swal({
                    title: "Please complete your setup",
...
function updateServiceWithComments(overwrite, commentsResponse, existingJsonStructure) {
    ...
    try {
        ...
        swal("Error Occurred", "Failed to parse response from ChatGPT.", "error");
        return;
    }
    ...
    try {
        ...
        swal("Error Occurred", "Failed to fetch existing service JSON.", "error");
        return;
    }
